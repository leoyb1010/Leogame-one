...
print "Bukov simulated integrated CPU benchmark: 30 enemies, 200 projectiles."
print "It executes a fixed frame count as fast as the host allows; it is not a wall-clock soak."
...
report="$project_root/build/reports/bukov-performance-e2e.log"
{
  ...
  print "benchmark_kind=simulated_integrated_cpu"
  print "wall_clock_soak=false"
  print "production_gameplay_path=false"
...
"$script_dir/apple-gradle" \
  ...
  -Dbukov.performance.e2e.seconds="$simulated_seconds_at_60hz" \
  --rerun-tasks \
  --no-daemon \
  2>&1 | tee -a "$report"
