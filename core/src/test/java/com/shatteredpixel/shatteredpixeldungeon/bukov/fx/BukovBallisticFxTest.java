...
public class BukovBallisticFxTest {
	...
	public void tracerGeometrySpansExactMuzzleToImpactVector() {
		...
		assertEquals(
				...
				0.0001f);
		assertTrue(geometry.coreThickness() >= 0.65f);
		assertTrue(geometry.coreThickness() <= 0.9f);
		assertTrue(geometry.glowThickness() >= 1.3f);
		assertTrue(geometry.glowThickness() <= 2f);
	...
	public void tracerHasBriefResidualWindowAndHardExpiry() {
		...
		assertTrue(BukovTracerFx.TRAVEL_SECONDS >= 0.08f);
	...
	public void tracerTailFollowsTheHeadWithoutDrawingTheWholeShotVector() {
		...
		assertEquals(16f, BukovTracerFx.tailLengthFor(geometry.length()), 0f);
		...
		assertEquals(34f, halfway.startX(), 0.0001f);
		assertEquals(50f, halfway.endX(), 0.0001f);
		assertEquals(16f, halfway.length(), 0.0001f);
		...
		assertEquals(84f, endpoint.startX(), 0.0001f);
		assertEquals(100f, endpoint.endX(), 0.0001f);
		assertEquals(16f, endpoint.length(), 0.0001f);
	...
	public void shortShotsUseAProportionalTailWithoutOvershootingTheMuzzle() {
		...
		assertEquals(5f, BukovTracerFx.tailLengthFor(geometry.length()), 0.0001f);
	...
	public void tracerDrawStateStaysVisibleAtSixtyAndOneHundredTwentyFps() {
		...
		assertEquals(1f, BukovTracerFx.headAlphaAt(
				BukovTracerFx.TRAVEL_SECONDS), 0f);
		assertTrue(BukovTracerFx.trailAlphaAt(0.14f) > 0f);
		assertTrue(BukovTracerFx.headAlphaAt(0.14f) > 0f);
	...
	public void tracerColorsAndOutlinedHeadAreActuallyOpaqueAndReadable() {
		...
		float coreThickness = BukovTracerFx.plan(
				...
				new PointF(16f, 0f),
				1f).coreThickness();
		assertTrue(BukovTracerFx.headWidthFor(coreThickness) >= 4f);
		assertTrue(BukovTracerFx.headWidthFor(coreThickness) < 6f);
		assertTrue(BukovTracerFx.headHeightFor(coreThickness) >= 1.5f);
		assertTrue(BukovTracerFx.headHeightFor(coreThickness) < 2.4f);
